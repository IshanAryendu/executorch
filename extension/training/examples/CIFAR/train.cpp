...
DEFINE_string(
    model_path,
    "/data/sandcastle/boxes/fbsource/fbcode/executorch/extension/training/examples/CIFAR/cifar10_model.pte",
...
DEFINE_string(
    train_data_path,
    "/data/sandcastle/boxes/fbsource/fbcode/executorch/extension/training/examples/CIFAR/cifar-10/extracted_data/train_data.bin",
    "Path to the combined training data file."); // Path to the combined train
                                                // data file
DEFINE_string(
    test_data_path,
    "/data/sandcastle/boxes/fbsource/fbcode/executorch/extension/training/examples/CIFAR/cifar-10/extracted_data/test_data.bin",
    "Path to the combined test data file."); // Path to the combined test data
                                            // file

DEFINE_string(
    ptd_save_path,
    "/data/sandcastle/boxes/fbsource/fbcode/executorch/extension/training/examples/CIFAR/CPP/",
...
DEFINE_int32(
    batch_size,
    1,
    "Batch size for training."); // Batch size for training (must match export
                                // batch size)
    ...
    for (int epoch = 0; epoch < FLAGS_num_epochs; epoch++)
    {
        ...
        for (size_t i = 0; i < dataset.size(); i += FLAGS_batch_size)
        {
            ...
            if (results.error() != Error::Ok)
            {
                ET_LOG(
                    Error,
                    "Failed to execute the forward method on batch starting at sample %zu",
                    i);
                return;
            }
            ...
            if (num_batches % 100 == 0)
            {
                ET_LOG(
                    Info,
                    "Epoch [%d/%d], Batch [%zu/%zu], Loss: %.4f, Data prep: %.2f ms, Train: %.2f ms",
                    epoch + 1,
                    FLAGS_num_epochs,
                    num_batches,
        ...
        ET_LOG(
            Info,
            "Epoch %d/%d Summary: Avg Loss: %.4f, Accuracy: %.2f%% (%zu/%zu), Time: %.2f s",
            epoch + 1,
            FLAGS_num_epochs,
            avg_loss,
            accuracy,
        ...
        executorch::extension::TensorPtr>> &test_dataset)
{
    ...
    ET_LOG(
        Info,
        "Final Test Results: Avg Loss: %.4f, Accuracy: %.2f%% (%zu/%zu), Time: %.2f s",
        test_avg_loss,
        test_accuracy,
        test_correct,
        test_total,
...
int main(int argc, char **argv)
{
    ...
    if (dir_fd == -1)
    {
        ...
        int result = mkdir(
            FLAGS_ptd_save_path.c_str(), 0755); // Create with permissions rwxr-xr-x
        if (result != 0)
        {
            ET_LOG(
